...
This is a guide for anyone who needs to share data with a statistician or data scientist. The target audiences are:
...
- [What you should deliver to the statistician](#what-you-should-deliver-to-the-statistician)
  - [The raw data](#the-raw-data)
  - [The tidy data set](#the-tidy-data-set)
  - [The code book](#the-code-book)
  - [How to code variables](#how-to-code-variables)
  - [The instruction list/script](#the-instruction-listscript)
- [What you should expect from the analyst](#what-you-should-expect-from-the-analyst)
...
1. Did not remove any data from the data set
1. Did not summarize the data in any way
...
units. See the [codebook template](codebook_template.md) in this repository for a starter document.
...
| Type | Description | Example | R class |
|------|-------------|---------|---------|
| Continuous | Measured on a quantitative scale; any fractional number | Weight in kg | `numeric` |
| Ordinal | Fixed, ordered levels (< 100) | Survey: poor / fair / good | `ordered factor` |
| Categorical | Multiple unordered categories | Sex: male / female | `factor` |
| Missing | Unobserved, unknown mechanism | — | `NA` |
| Censored | Unobserved, known mechanism | Below detection limit | `NA` + censor flag |

[Ordinal data](http://en.wikipedia.org/wiki/Ordinal_data) have a fixed, small (< 100) number of levels but are ordered.
[Categorical data](http://en.wikipedia.org/wiki/Categorical_variable) have multiple categories that aren't ordered.
[Missing data](http://en.wikipedia.org/wiki/Missing_data) are unobserved values where you don't know the mechanism — code these as `NA`.
[Censored data](http://en.wikipedia.org/wiki/Censoring_\(statistics\)) are observations where you know the missingness mechanism on some level
(e.g., a measurement below a detection limit, or a patient lost to follow-up). Code them as `NA` and add a companion
`VariableNameCensored` column with values `TRUE` / `FALSE`. In the code book, explain why those values are missing.
It is absolutely critical to report to the analyst any known reason why some data are missing. You should also not
[impute](http://en.wikipedia.org/wiki/Imputation_\(statistics\))/make up/throw away missing observations.

In general, try to avoid coding categorical or ordinal variables as numbers. When you enter the value for sex in the tidy
data, it should be `"male"` or `"female"`. Ordinal values should be `"poor"`, `"fair"`, and `"good"`, not `1`, `2`, `3`.
This avoids potential mixups about direction of effects and helps identify coding errors.

Always encode every piece of information about your observations using text. For example, if you are storing data in Excel
and use colored text or cell background formatting to indicate information about an observation ("red variable entries were
observed in experiment 1"), that information will **not** be exported (and will be lost!) when the data is saved as raw text.
Every piece of data should be encoded as actual text that can be exported.
...
That means, when you submit your paper, reviewers and the rest of the world should be able to exactly replicate
...
a couple of times and see if the code produces the same output. See the [R tidy data template](tidy_data_template.R) in this
repository for a reusable starting point.
...
The following R packages are particularly useful when preparing tidy data for a statistician:

| Package | Purpose | Install |
|---------|---------|---------|
| [tidyr](https://tidyr.tidyverse.org/) | Reshape data between wide and long formats | `install.packages("tidyr")` |
| [dplyr](https://dplyr.tidyverse.org/) | Data manipulation (filter, select, mutate, summarise) | `install.packages("dplyr")` |
| [readr](https://readr.tidyverse.org/) | Fast and consistent CSV/TSV reading | `install.packages("readr")` |
| [janitor](https://sfirke.github.io/janitor/) | Clean column names and tabulate data | `install.packages("janitor")` |
| [skimr](https://docs.ropensci.org/skimr/) | Quick summary statistics for code book creation | `install.packages("skimr")` |
| [dataMaid](https://github.com/ekstroem/dataMaid) | Automated code book / data validation reports | `install.packages("dataMaid")` |
...
raw  <- read_csv("raw_data.csv")    # read raw data
tidy <- raw |>
  janitor::clean_names() |>        # standardize column names
  tidyr::pivot_longer(              # reshape wide → long if needed
    cols      = starts_with("time_"),
    names_to  = "time_point",
    values_to = "measurement"
  )

write_csv(tidy, "tidy_data.csv")
```

A full, annotated script is available in [`tidy_data_template.R`](tidy_data_template.R).

## Templates

| File | Description |
|------|-------------|
| [`tidy_data_template.R`](tidy_data_template.R) | Annotated R script for reading, cleaning, and exporting tidy data |
| [`codebook_template.md`](codebook_template.md) | Markdown template for documenting variables, study design, and summary choices |
